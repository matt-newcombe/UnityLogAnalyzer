...
class APIClient {
    ...
    async getErrorWarningCounts() {
        try {
            const db = await this.getDatabase();
            return await db.getErrorWarningCounts(this.currentLogId);
        } catch (error) {
            console.error('Error getting error/warning counts:', error);
            throw error;
        }
    }
    ...
    async getScriptCompilation() {
        try {
            const db = await this.getDatabase();
            return await db.getScriptCompilation(this.currentLogId);
        } catch (error) {
            console.error('Error getting script compilation:', error);
            throw error;
        }
    }
}
